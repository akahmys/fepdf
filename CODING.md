...
| **UI-5** | Localisation | No user-facing string literal in the source; all through the locale keys. Two checks: a literal handed to a call that shows it, and any literal that reads as prose — the second exists because the first reported zero while fifteen strings were in English | `scripts/audit/strings.py` via `verify_compliance.sh`; `locale.rs` separately holds the two key sets equal |
