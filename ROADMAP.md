...
| **9** Text | **Re-measured, and the row was false in a way its own wording hid.** It read *"every page of every sample yields its text"*: 228 of 7727 pages yield none. Two causes, and neither was recorded. **The first was a path.** `Adobe-Japan1-UCS2` is built from `external/adobe-cmaps/.../cid2code.txt`, found through a *relative* default, so the same document extracted differently depending on the directory the process started in — `bokutokitan.pdf` gave 64,556 glyphs' worth of text from the repository root and an empty string from `crates/fepdf`. For any consumer of the crate the directory does not exist at all. There is one root now with one layout (`fepdf_font::resources`), the source-tree fallback is anchored to `CARGO_MANIFEST_DIR` rather than the working directory, and `FEPDF_RESOURCES` is exclusive when set. `resource_dir` was defined twice with two defaults and one variable overriding both, so a value that found the CMaps sent the font loader to `<cmaps>/fonts`; that is gone. **The second is a gap, and it is bigger than the page count suggested.** Counting pages hid partial loss: a page that yields *some* text looked extracted. Counting glyphs, across the corpus **82,601 of 12,305,585 have no Unicode value** — `bokutokitan.pdf` loses **58%** of its 65,102, `volvo_xc90.pdf` 12%, `intel_sdm.pdf` 0.3%. It was 85,982 and 63% until one of the causes was found: `cid2code.txt` writes a trailing `v` on a value that appears in a vertical-writing CMap, the reader passed `"3001v"` to `from_str_radix` and dropped the row, and CID 7887 is 、 — so a vertically set novel lost its punctuation. 186 CIDs of 23,060, **3,381 glyphs recovered**, and the column is now chosen by whether it reads rather than by whether it is non-empty. **That is a fifteenth of the loss, not the bulk of it.** **Where the rest fails is now reported rather than guessed at.** Every route that can name a character carries a `UnicodeSource`, and the decision says which one was reached: of the 82,601, **78,711 reached no route at all**, **2,801 were named and then discarded** by the private-use and circled-number filter, and 1,089 reached the encoding and came back empty. The 2,801 is the filter this table could not see: Adobe-Japan1 defines 128 CIDs that *are* circled numbers, and they carry no Shift-JIS and no EUC encoding at all — so a Unicode-based route reaching them is right and a legacy one is not, which is a judgement about the route and not about the value. **Labelling found three chains where the row assumed one**: `decode_next` tries `/ToUnicode` and the encoding, then falls through to `unicode_for`, which tries both again, and each of the three carries its own copy of that filter. Every one of them is recorded now, per page, as a `9.10.2` `Violation` naming the count — and where the CMap collections are absent the decision lists the paths that were searched, because *"not installed"* is an ordinary answer for data this engine does not carry and an empty string is not. **An empty extraction no longer means two things**: 24 of `fugaku.pdf`'s 25 pages are drawn in vector paths and 2 pages across the corpus are blank, and those record nothing, correctly. What remains open is the mapping itself. |
