...
| **11** Transparency | **Re-measured, and it was three things rather than one.** Blend modes (11.3.5) and constant alpha (`/ca`, `/CA`) do reach the backend — pinned now by `transparency_test.rs` so that "not re-measured" cannot happen to them twice. **A soft mask did not.** `/SMask` in an `/ExtGState` was read into the interpreter's state and used by nothing: the write is at `ops/state.rs` and there is no reader, and `RenderBackend` has no soft-mask entry point at all. Measured with a `/S /Luminosity` mask whose group paints solid black — 11.6.5.2 makes that mask 0 everywhere, so the content it covers contributes nothing — and the covered rectangle was filled at full strength with no `Decision` beside it. **160 firings across the nine samples, all of them in `volvo_xc90.pdf`**, whose 415 pages have been drawn with 160 masks ignored. **Transparency groups (11.6.6) are not read.** A form carrying `/Group << /S /Transparency /I true /K true >>` produces backend calls identical to the same form without the entry — the same list in the same order — so isolation and knockout are absent rather than approximate. Both are recorded now, the group one only when `/I` or `/K` asks for something, because a plain group is what Illustrator and InDesign put around almost every form and a decision on each would say nothing by saying it everywhere: **0 firings on the samples.** |
